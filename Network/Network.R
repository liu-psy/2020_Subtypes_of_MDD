...
f2 <- function(data, title, pred) {
  network <- qgraph(data, title = title, pie = pred, layout = L, title.cex = 2.5,
...
centra_list <- centra %>% group_by(graph, measure) %>% 
  select(graph, measure, node, value) %>% group_split()
...
decribe_node <- function(group) {
  # Sort by centrality from large to small
...
# Betweenness
rank_nodes$LOI_Betweenness; rank_nodes$SAI_Betweenness
# Clossness
...
p1 <- f6(kboot1b, "Sampled People (LOI)")
p2 <- f6(kboot2b, "Sampled People (SAI)")
